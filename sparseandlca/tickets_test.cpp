...
int parent[N][M], depth[N], max_ticket[N][M];
void dfs(int node, int par, int dep, int ticket_cost) {
    parent[node][0] = par;
    depth[node] = dep;
...
int maxTicketOnPath(int u, int v) {
    ...
    for (int j = 0; j < M; j++) {
        if (diff & (1 << j)) {
            max_cost = max(max_cost, max_ticket[u][j]);
            u = parent[u][j];
        }
    }
    ...
    for (int j = M - 1; j >= 0; j--) {
        if (parent[u][j] != parent[v][j]) {
            max_cost = max({max_cost, max_ticket[u][j], max_ticket[v][j]});
            u = parent[u][j];
            v = parent[v][j];
        }
    }
    // Finally, check the last edge leading to the LCA
    return max({max_cost, max_ticket[u][0], max_ticket[v][0]});
}
int main() {
    abdo;
    int n;
